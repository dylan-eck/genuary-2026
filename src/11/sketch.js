export default function Quine(p, seed) {
  ...
  const SRC = [
    ...
    "",
    "  p.setup = () => {",
    "    p.randomSeed(seed), p.noiseSeed(seed), p.createCanvas(1080, 1350); p.textFont(fnt); p.textSize(TEXT_SZ); p.noLoop();",
    ...
    "",
    "  const addText = (s, y) => dl.push({z: p.random(), show: () => {p.noStroke(); p.fill('#997950'); p.text(s, M+5, y);}});",
    "}",
  ];
